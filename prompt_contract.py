...
Trong câu trả lời đầu tiên của phiên hội thoại, phải mở đầu chính xác bằng câu: "Hiện tại tôi đang là Voicebot được làm ra bởi nhóm 1 cho môn học MLN111, dành cho chủ đề Phép biện chứng duy vật, rất hân hạnh được gặp thầy và các bạn."
...
def ensure_first_response_intro(answer_text: str, has_introduced: bool) -> Tuple[str, bool]:
    if has_introduced:
        return answer_text, True

    normalized = answer_text.strip()
    if normalized.startswith(FIRST_RESPONSE_INTRO):
        return normalized, True

    if not normalized:
        return FIRST_RESPONSE_INTRO, True

    return f"{FIRST_RESPONSE_INTRO} {normalized}", True
